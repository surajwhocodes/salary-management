...
function Field({
  ...
  return (
    <div className="space-y-1">
      <label className="flex items-center gap-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wide">
...
function Section({
  ...
  return (
    <div className="bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 bg-slate-50/60 px-5 py-3.5 border-slate-100 border-b">
...
export default function EditEmployeePage() {
  ...
  if (loadError) {
    return (
      <main className="mx-auto px-4 py-16 max-w-5xl">
        <div className="flex items-start gap-4 bg-rose-50 p-6 border border-rose-200 rounded-2xl text-rose-800">
          ...
          <div>
            ...
            <Link
              href="/employees"
              className="inline-flex items-center gap-1.5 mt-3 font-medium text-sm underline"
            >
  ...
  if (!employee) {
    return (
      <main className="space-y-6 mx-auto px-4 py-12 max-w-5xl animate-pulse">
        <div className="bg-slate-200 rounded w-32 h-5" />
        <div className="bg-slate-200 rounded-2xl h-28" />
        <div className="gap-6 grid lg:grid-cols-[300px_1fr]">
          <div className="space-y-6">
            <div className="bg-slate-200 rounded-2xl h-56" />
            <div className="bg-slate-200 rounded-2xl h-44" />
          </div>
          <div className="space-y-6">
  ...
  return (
    <main className="mx-auto px-4 sm:px-6 py-8 pb-32 max-w-5xl">
      {/* Back */}
      <Link
        href="/employees"
        className="group inline-flex items-center gap-2 mb-5 text-slate-500 hover:text-slate-900 text-sm transition-colors"
      >
        <ArrowLeft
          size={14}
      ...
      {/* Hero header */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 shadow-lg mb-6 p-5 border border-slate-700/40 rounded-2xl text-white">
        <div className="flex flex-wrap items-center gap-4">
          <div
            className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${avatarColor(employee.firstName)} flex items-center justify-center font-bold text-white text-lg shrink-0 shadow`}
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="mt-0.5 font-bold text-xl truncate">
              {liveName || "—"}
            </h1>
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              ...
              {liveDept && (
                <>
                  ...
                  <span className="text-slate-300 text-xs">{liveDept}</span>
                </>
              )}
              {liveEmail && (
                <>
                  <span className="text-slate-600 text-xs">·</span>
                  <span className="text-slate-400 text-xs">{liveEmail}</span>
                </>
              )}
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${STATUS_STYLES[liveStatus] ?? "bg-slate-100 text-slate-600 border-slate-200"}`}
              >
                {liveStatus}
              </span>
            </div>
          ...
          <div className="text-right shrink-0">
            ...
            <p className="mt-0.5 font-bold text-emerald-400 text-2xl">
              {fmt(liveNet, g("currency") || "USD")}
            </p>
            <p className="mt-0.5 text-slate-500 text-xs">annual</p>
          </div>
        </div>
      </div>
      ...
      {/* Banners */}
      {saved && (
        <div className="flex items-center gap-3 bg-emerald-50 mb-4 p-3.5 border border-emerald-200 rounded-xl font-medium text-emerald-800 text-sm">
          <CheckCircle2 size={16} className="text-emerald-600 shrink-0" />
          Changes saved — returning to employee list…
        </div>
      )}
      {saveError && (
        <div className="flex items-start gap-3 bg-rose-50 mb-4 p-3.5 border border-rose-200 rounded-xl text-rose-800 text-sm">
      ...
      {dirty && !saved && (
        <div className="flex items-center gap-2 bg-amber-50 mb-4 px-3.5 py-2.5 border border-amber-200 rounded-xl text-amber-700 text-sm">
      ...
      <form onSubmit={handleSubmit}>
        <div className="items-start gap-6 grid lg:grid-cols-[300px_1fr]">
          {/* ── LEFT SIDEBAR ────────────────────────────────────────────── */}
          <div className="space-y-6">
            {/* Profile identity */}
            <Section
            ...
            >
              <div className="space-y-4">
                <Field
                  ...
                  name="employeeId"
                  type="text"
                  defaultValue={g("employeeId")}
                  readOnly
                  hint="System-assigned"
                />
            ...
            <Section
              title="Employment"
              description="Role & contract type"
          ...
          <div className="space-y-6">
            ...
            >
              <div className="gap-4 grid sm:grid-cols-2">
                ...
                <Field
                  ...
                  name="allowance"
                  type="number"
                  defaultValue={g("allowance")}
                  onChange={(v) => handleChange("allowance", v)}
                  hint="Annual allowance"
                />
                <Field
                  label="Tax"
                  icon={DollarSign}
                  ...
                  hint="Annual deduction"
                />
              </div>

              {/* Net salary summary card */}
              <div className="flex justify-between items-center bg-gradient-to-r from-slate-900 to-slate-800 mt-5 p-4 rounded-xl text-white">
                ...
                <div className="text-right">
                  ...
                  <input
                    type="hidden"
                    name="netSalary"
                    value={live.netSalary ?? String(employee.netSalary)}
                    readOnly
                  />
                </div>
              </div>
            ...
            {liveNet > 0 &&
              (() => {
                ...
                const pct = (v: number) =>
                  gross > 0 ? Math.round((v / gross) * 100) : 0;
                return (
                  <div className="bg-white shadow-sm p-5 border border-slate-200 rounded-2xl">
                    <p className="mb-4 font-semibold text-slate-500 text-xs uppercase tracking-wide">
        ...
        <div className="right-0 bottom-0 left-0 z-20 fixed flex justify-between items-center gap-4 bg-white/90 shadow-lg backdrop-blur-sm px-6 py-3.5 border-slate-200 border-t">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <Link
              href="/employees"
              className="hover:bg-slate-50 px-4 py-2.5 border border-slate-300 hover:border-slate-400 rounded-xl font-medium text-sm transition-all duration-200"
            >
              Cancel
            </Link>
            <button
              ...
              className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-60 shadow-sm px-6 py-2.5 rounded-xl font-medium text-white text-sm transition-all duration-200 disabled:cursor-not-allowed"
            >
              {saving ? (
                <>
